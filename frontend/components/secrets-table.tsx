...
import { Eye, EyeOff, Copy, MoreHorizontal, Clock, Plus, Loader2 } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function SecretsTable({ 
  ...
  const [decryptingSecrets, setDecryptingSecrets] = useState<Record<string, boolean>>({})
  
  // Wallet hooks for authentication and encryption
  const { publicKey, connected } = useWallet()
  const { isInitialized, handleSignMessage, decryptData } = useWalletEncryption()
  ...
  useEffect(() => {
    async function fetchSecrets() {
      ...
      try {
        ...
        // This allows the backend to return only secrets accessible by this wallet
        const walletAddress = publicKey ? publicKey.toBase58() : null;
          console.log(`Fetching secrets for project: ${projectId}, environment: ${environment}${walletAddress ? `, wallet: ${walletAddress}` : ''}`);
        
        // Fetch environment ID if needed
        let environmentId = environment;
        if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(environment)) {
          // If it's not a UUID format, we need to fetch the environment ID first
          console.log(`Environment parameter "${environment}" is not in UUID format, fetching environment ID...`);
          try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3002';
            const envResponse = await fetch(`${apiUrl}/api/environments?project_id=${projectId}`);
            if (!envResponse.ok) {
              throw new Error(`Failed to fetch environments: ${envResponse.status}`);
            }
            const data = await envResponse.json();
            // Find the environment ID for this name
            const env = data.environments?.find((e: any) => e.name === environment);
            if (env?.id) {
              environmentId = env.id;
              console.log(`Found environment ID ${environmentId} for environment "${environment}"`);
            } else {
              console.error(`Could not find environment ID for "${environment}"`);
            }
          } catch (err) {
            console.error("Error fetching environment ID:", err);
          }
        }
        ...
        // Only include walletAddress if we have one
        let url = `${apiUrl}/api/secrets?project_id=${projectId}&environment_id=${environmentId}`;
        if (walletAddress) {
        ...
        console.log(`Fetched ${data.secrets?.length || 0} secrets from API`);        // Map backend response to our Secret type
        const formattedSecrets: Secret[] = data.secrets?.map((secret: any) => {
          ...
          // Make sure we get the full data with encrypted_value and iv
          if (!secret.encrypted_value || !secret.iv) {
            // We might need to make an additional request to get the full secret data
            console.warn(`Secret ${secret.id} (${secret.name}) is missing encrypted_value or iv:`, {
          ...
          return {
            id: secret.id,
            name: secret.name,
            value: secret.encrypted_value || '', // This might be missing due to how data is returned
            type: secret.type?.toLowerCase() || 'string', // Convert backend type to our UI type
            iv: secret.iv || '', // This might be missing due to how data is returned
        ...
        console.log(`Loaded ${formattedSecrets.length} secrets for project: ${projectId}, environment: ${environment}`);
  ...
  const toggleSecretVisibility = async (secret: Secret) => {
    ...
    try {
      ...
      if (!isInitialized) {
        try {
          ...
          if (error instanceof Error && error.message.includes('User rejected')) {
            throw new Error('You cancelled the signature request. Authentication is required to decrypt secrets.');
          } else {
      ...
      try {
        ...
        console.log(`Successfully decrypted secret ${secret.id}, value: ${decryptedValue?.substring(0, 10)}...`);
        
        // Update secret with decrypted value
        const updatedSecrets = secrets.map(s => 
          s.id === secret.id ? {...s, decryptedValue: decryptedValue || ''} : s
        );
        ...
        let errorMessage = "Could not decrypt this secret. The wallet signature may be required.";
        
        if (error instanceof Error) {
          if (error.message.includes('cancelled') || error.message.includes('rejected')) {
            errorMessage = "You cancelled the signature request. Authentication is required to decrypt.";
          } else if (error.message.includes('signature verification failed')) {
            errorMessage = "Signature verification failed. This secret might be encrypted with a different wallet.";
          } else if (error.message.includes('key')) {
            errorMessage = "Encryption key error. Please reconnect your wallet and try again.";
          } else {
            ...
            <TableRow>
              <TableCell colSpan={5} className="h-24 text-center">
                <div className="text-center">
                  <h3 className="text-lg font-medium">No secrets found in {environment}</h3>
                  ...
                  {!searchQuery && (
                    <Button className="mt-4" onClick={() => {
                      document.dispatchEvent(
                        new CustomEvent('open-add-secret-dialog', { detail: { environment } })
                      )
                    }}>
                  ...
                  {!error && !isInitialized && (
                    <p className="mt-2 text-xs text-muted-foreground">
                      Connect your wallet and sign a message to reveal secrets
                    </p>
                  )}
                </div>
              </TableCell>
